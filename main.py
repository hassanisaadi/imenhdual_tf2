...
def model_train(model_name, lr):
    with load_data(filepath=args.hdf5_file) as data:
        eval_files_YL = sorted(glob('{}/YL*.png'.format(args.eval_set)))
        eval_files_XL = sorted(glob('{}/XL*.png'.format(args.eval_set)))
        eval_files_XR = sorted(glob('{}/XR*.png'.format(args.eval_set)))
